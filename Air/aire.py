...
Y = data_aire_df.iloc[:, 6].values #8 si es data_level_pollution y 6 si es level_pollution
...
clusterer = sklearn.cluster.DBSCAN(eps=0.12).fit(norm_aire) # 0.129 si es data_level_pollution_o3, sino 0.12
graph = mapper.map(
    projected_data,
    norm_aire,
    cover=km.Cover(n_cubes=30, perc_overlap=0.15), # 20 y 0.15 si es data_level_pollution_o3
