...
Y = data_aire_df.iloc[:, 6].values #8 for data_level_pollution and 6 for level_pollution
...
clusterer = sklearn.cluster.DBSCAN(eps=0.12).fit(norm_aire) # 0.129 for data_level_pollution_o3, if not 0.12
graph = mapper.map(
    projected_data,
    norm_aire,
    cover=km.Cover(n_cubes=30, perc_overlap=0.15), # 20 and 0.15 for data_level_pollution_o3, if not 30
    clusterer=sklearn.cluster.DBSCAN(eps=0.12),
)
#print(graph)
...
mapper.visualize(
    graph,
    path_html="Mapper_Aire_General_cla.html",
    title="Aire",
    custom_tooltips=Y,
    color_values=Y,
